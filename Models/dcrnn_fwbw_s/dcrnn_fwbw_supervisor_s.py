...
from Models.dcrnn_fwbw_s.dcrnn_fwbw_model_s import DCRNNModelF, DCRNNModelB
...
class DCRNNSSupervisor(AbstractModel):
    ...
    def __init__(self, is_training=False, **kwargs):
        ...
        with tf.name_scope('TrainF'):
            with tf.variable_scope('DCRNN', reuse=False):
                self.train_model_f = DCRNNModelF(is_training=True, scaler=scaler,
                                                 batch_size=self._data_kwargs['batch_size'],
                                                 adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('ValF'):
            with tf.variable_scope('DCRNN', reuse=True):
                self.val_model = DCRNNModelF(is_training=False, scaler=scaler,
                                             batch_size=self._data_kwargs['val_batch_size'],
                                             adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('EvalF'):
            with tf.variable_scope('DCRNN', reuse=True):
                self.eval_model = DCRNNModelF(is_training=False, scaler=scaler,
                                              batch_size=self._data_kwargs['eval_batch_size'],
                                              adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('TestF'):
            with tf.variable_scope('DCRNN', reuse=True):
                self.test_model = DCRNNModelF(is_training=False, scaler=scaler,
                                              batch_size=self._data_kwargs['test_batch_size'],
                                              adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('TrainB'):
            with tf.variable_scope('DCRNN', reuse=False):
                self.train_model_b = DCRNNModelB(is_training=True, scaler=scaler,
                                                 batch_size=self._data_kwargs['batch_size'],
                                                 adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('ValB'):
            with tf.variable_scope('DCRNN', reuse=True):
                self.val_model_b = DCRNNModelB(is_training=False, scaler=scaler,
                                               batch_size=self._data_kwargs['val_batch_size'],
                                               adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('EvalB'):
            with tf.variable_scope('DCRNN', reuse=True):
                self.eval_model_b = DCRNNModelB(is_training=False, scaler=scaler,
                                                batch_size=self._data_kwargs['eval_batch_size'],
                                                adj_mx=self._data['adj_mx'], **self._model_kwargs)

        with tf.name_scope('TestB'):
            with tf.variable_scope('DCRNN', reuse=True):
                self.test_model_b = DCRNNModelB(is_training=False, scaler=scaler,
                                                batch_size=self._data_kwargs['test_batch_size'],
                                                adj_mx=self._data['adj_mx'], **self._model_kwargs)
        ...
        # fw decoder
        preds_fw = self.train_model_f.outputs_fw
        labels_fw = self.train_model_f.labels_fw[..., :output_dim]

        # bw encoder
        enc_preds_bw = self.train_model_b.enc_outputs_bw
        enc_labels_bw = self.train_model_b.enc_labels_bw[..., :output_dim]
