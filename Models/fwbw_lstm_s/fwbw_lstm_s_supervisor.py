...
class FwbwLstmSRegression(AbstractModel):

    def __init__(self, is_training=False, **kwargs):
        super(FwbwLstmSRegression, self).__init__(**kwargs)
        ...
        self._checkpoints = ModelCheckpoint(
            self._log_dir + "best_fw_model.hdf5",
        ...
        self.callbacks_list_bw = []
        self._checkpoints_bw = ModelCheckpoint(
            self._log_dir + "best_bw_model.hdf5",
            monitor='val_loss', verbose=1,
            save_best_only=True,
            mode='auto', period=1)
        self.callbacks_list_bw = [self._checkpoints_bw]
        self._earlystop_bw = EarlyStopping(monitor='val_loss', patience=self._train_kwargs.get('patience'),
                                           verbose=1, mode='auto')
        self.callbacks_list_bw.append(self._earlystop_bw)
        self._time_callback_bw = TimeHistory()
        self.callbacks_list_bw.append(self._time_callback_bw)
    ...
    def construct_fwbw_lstm(self):
        ...
        bw_outputs = Dense(256, )(bw_outputs)
        bw_outputs = Dropout(0.5)(bw_outputs)
        bw_outputs = Dense(128, )(bw_outputs)
        bw_outputs = Dropout(0.5)(bw_outputs)
    ...
    def train(self):
        ...
        training_fw_history = self.fw_model.fit(x=self._data['x_train'],
                                                y=self._data['y_train_1'],
                                                batch_size=self._batch_size,
                                                epochs=self._epochs,
                                                callbacks=self.callbacks_list,
                                                validation_data=(self._data['x_val'], self._data['y_val_1']),
                                                shuffle=True,
                                                verbose=2)

        if training_fw_history is not None:
            self.plot_training_history(training_fw_history, tag='fw')
            self.save_model_history(times=self._time_callback.times, model_history=training_fw_history)

        training_bw_history = self.bw_model.fit(x=self._data['x_train'],
                                                y=self._data['y_train_2'],
                                                batch_size=self._batch_size,
                                                epochs=self._epochs,
                                                callbacks=self.callbacks_list_bw,
                                                validation_data=(self._data['x_val'], self._data['y_val_2']),
                                                shuffle=True,
                                                verbose=2)
    ...
    def load(self):
        self.fw_model.load_weights(self._log_dir + 'best_fw_model.hdf5')
        self.bw_model.load_weights(self._log_dir + 'best_bw_model.hdf5')
