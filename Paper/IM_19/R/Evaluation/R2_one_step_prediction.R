data<-read.table("/home/anle/Dropbox/Research/TM_estimation_RNN/Paper/IM_19/R/Evaluation/R2_one_step_prediction.csv",header=T,sep=",")
max_x <- max(data$p)
min_x <- min(data$p)
...
setEPS()
postscript("/home/anle/Dropbox/Research/TM_estimation_RNN/Paper/IM_19/R/Evaluation/R2_one_step_prediciton.eps",height=3, width=4.5)
par(mar=c(4, 4.5, 0, 1) + 0.1)
par(mgp = c(3, 1, 0))

plot_colors <-  c("dodgerblue4", "black", "red")

plot(c(min_x,max_x),c(0, 1), type="n", cex.lab = 1.5, cex.axis=1.5, xlab="Percentage of monitored flows (%)", ylab=expression(R^2))
