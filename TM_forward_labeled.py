...
def forward_rnn_labeled_features(raw_data, dataset_name='Abilene24s'):
...
def try_hyper_parameter(raw_data, dataset_name='Abilene24s'):
    ...
    for forward_weight in np.nditer(forward_weight_range):
        hyperparams[3] = forward_weight
        ...
        error = np.expand_dims(np.array([forward_weight, a_nmae, a_nmse, pred_confident, err_rat]), axis=0)
    ...
    np.savetxt('./Errors/[%s][lookback_%i]Errors_by_forward_weight_2_2.csv' % (test_name, look_back), errors, delimiter=',')
...
if __name__ == "__main__":
    ...
    # forward_backwrd_rnn_one_features(raw_data=Abilene24s_data, dataset_name='Abilene24s')
    ...
    # forward_rnn_labeled_features(raw_data=Abilene24s_data, dataset_name='Abilene24s')
