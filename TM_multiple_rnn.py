...
def prepare_initialized_input(training_data, look_back_by_day, sampling_itvl):
    day_size = 24 * (60 / sampling_itvl)

    last_days_data = training_data[-(look_back_by_day * day_size):, :]
    initial_input = prepare_input_online_by_ts(last_days_data, 0, look_back_by_day, sampling_itvl)
    return initial_input


def prepare_input_online_by_ts(tm, current_ts, look_back_by_day, sampling_itvl):
    day_size = 24 * (60 / sampling_itvl)
    input = tm[-(day_size * look_back_by_day) + current_ts::day_size, :].T
...
def predict_with_loss_test_lookback_by_day(test_set, based_data, look_back_by_day, rnn_model, sampling_ratio):
    """
    ...
    # The TF array for random choosing the measured flows
    tf = np.array([True, False])
    # measured_matrix = np.array([True] * test_set.shape[1])
    # measured_matrix = np.reshape(measured_matrix, (1, test_set.shape[1]))
    measured_matrix = np.empty((0, test_set.shape[1]))

    predicted_matrix = np.empty((0, test_set.shape[1]))

    # Predict the TM from time slot look_back
    for tslot in range(0, test_set.shape[0], 1):
        # print ('--- Predict at timeslot %i ---' % tslot)

        # Create 3D input for rnn
        rnn_input = prepare_input_online_by_ts(ret_tm.T, tslot % day_size, look_back_by_day, sampling_itvl=5)

        # Get the TM prediction of next time slot
        predictX = rnn_model.predict(rnn_input)
        if len(predictX.shape) > 2:
            predictX = np.reshape(predictX, (predictX.shape[0], predictX.shape[1]))
        ...
        # Concatenating new_input into current rnn_input
        ret_tm = np.concatenate([ret_tm, new_input], axis=1)
        predicted_matrix = np.concatenate([predicted_matrix, new_input.T], axis=0)

    return predicted_matrix, measured_matrix
...
def test_lookback_by_day(raw_data, dataset_name):
    test_name = 'lookback_by_day'
    splitting_ratio = [0.7, 0.3]
    a_lookback = range(3, 6, 1)
    errors = np.empty((0, 3))

    model_recorded_path = './Model_Recorded/' + dataset_name + '/lookback_by_day/'
    ...
    for look_back in a_lookback:
        ...
        if not os.path.isfile(trainX_path):
            ...
            trainX, trainY = create_xy_set_over_day(data=training_set, look_back_days=look_back, sampling_itvl=5)
        ...
        rnn = RNN(
            saving_path=model_recorded_path + 'hidden_%i_lookback_%i_epoch_%i_test_1/' % (
            HIDDEN_DIM, look_back, N_EPOCH),
            raw_data=raw_data,
            look_back=look_back,
            n_epoch=N_EPOCH,
        ...
        if os.path.isfile(path=rnn.saving_path + 'model.json'):
            ...
            ############################################################################################################
            # Test 1: input shape = (time_step, 1)
            trainX = np.reshape(trainX, (trainX.shape[0], trainX.shape[1], 1))

            ############################################################################################################
            # Test 2: input shape = (1, features)
        ...
        based_data = training_set[-(look_back * 288):, :]

        for sampling_ratio in sampling_ratioes:
            pred_tm, measured_matrix = predict_with_loss_test_lookback_by_day(testing_set,
                                                                              based_data=based_data,
                                                                              look_back_by_day=look_back,
                                                                              rnn_model=rnn.model,
...
if __name__ == "__main__":
    ...
    test_lookback_by_day(raw_data=Abilene24s_data, dataset_name='Abilene24s')
