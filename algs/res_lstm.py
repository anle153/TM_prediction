...
def prepare_input_online_prediction(data, labels):
    ...
    dataX = np.zeros(shape=(data.shape[1], Config.RES_LSTM_STEP, 2))
...
def ims_tm_prediction(init_data, model, init_labels):
    multi_steps_tm = np.zeros(shape=(init_data.shape[0] + Config.RES_LSTM_IMS_STEP, init_data.shape[1]))
    multi_steps_tm[0:Config.RES_LSTM_STEP, :] = init_data

    labels = np.zeros(shape=(init_labels.shape[0] + Config.RES_LSTM_IMS_STEP, init_labels.shape[1]))
    labels[0:Config.RES_LSTM_STEP, :] = init_labels

    for ts_ahead in range(Config.RES_LSTM_IMS_STEP):
        rnn_input = prepare_input_online_prediction(data=multi_steps_tm[ts_ahead:ts_ahead + Config.RES_LSTM_STEP],
                                                    labels=labels[ts_ahead:ts_ahead + Config.RES_LSTM_STEP])
...
def predict_lstm_nn(init_data, test_data, model):
    ...
    ims_tm = np.zeros(shape=(test_data.shape[0] - Config.RES_LSTM_IMS_STEP + 1, test_data.shape[1]))
    ...
    for ts in tqdm(range(test_data.shape[0])):
        ...
        if Config.RES_LSTM_IMS and (ts <= test_data.shape[0] - Config.RES_LSTM_IMS_STEP):
            ims_tm[ts] = ims_tm_prediction(init_data=tm_pred[ts:ts + Config.RES_LSTM_STEP, :],
                                           model=model,
                                           init_labels=labels[ts:ts + Config.RES_LSTM_STEP, :])

        # Create 3D input for rnn
        rnn_input = prepare_input_online_prediction(data=tm_pred[ts: ts + Config.RES_LSTM_STEP],
                                                    labels=labels[ts: ts + Config.RES_LSTM_STEP])
        ...
        sampling = np.random.choice(tf_a, size=(test_data.shape[1]),
                                    p=[Config.RES_LSTM_MON_RAIO, 1 - Config.RES_LSTM_MON_RAIO])

        labels[ts + Config.RES_LSTM_STEP] = sampling
        ...
        tm_pred[ts + Config.RES_LSTM_STEP] = new_input

    return tm_pred[Config.RES_LSTM_STEP:, :], labels[Config.RES_LSTM_STEP:, :], ims_tm
...
def build_model(input_shape):
    ...
    net = lstm(input_shape=input_shape,
               hidden=Config.RES_LSTM_HIDDEN_UNIT,
               drop_out=Config.RES_LSTM_DROPOUT,
...
def train_res_lstm(data):
    ...
    if not Config.RES_LSTM_VALID_TEST or \
        ...
        if os.path.isfile(path=lstm_net.checkpoints_path + 'weights-{:02d}.hdf5'.format(Config.RES_LSTM_N_EPOCH)):
            lstm_net.load_model_from_check_point(_from_epoch=Config.RES_LSTM_BEST_CHECKPOINT)
    ...
    results_summary = pd.DataFrame(index=range(Config.RES_LSTM_TESTING_TIME),
...
def ims_tm_test_data(test_data):
    ims_test_set = np.zeros(
        shape=(test_data.shape[0] - Config.RES_LSTM_IMS_STEP + 1, test_data.shape[1]))

    for i in range(Config.RES_LSTM_IMS_STEP - 1, test_data.shape[0], 1):
        ims_test_set[i - Config.RES_LSTM_IMS_STEP + 1] = test_data[i]
...
def test_res_lstm(data):
    ...
    with tf.device('/device:GPU:{}'.format(Config.GPU)):
        ...
        lstm_net = load_trained_model(input_shape, Config.RES_LSTM_BEST_CHECKPOINT)
...
def prepare_test_set(test_data2d, test_data_normalized2d):
    ...
    idx = np.random.random_integers(Config.RES_LSTM_STEP,
                                    test_data2d.shape[0] - day_size * Config.RES_LSTM_TEST_DAYS - 10)

    test_data_normalize = test_data_normalized2d[idx:idx + day_size * Config.RES_LSTM_TEST_DAYS]
    init_data_normalize = test_data_normalized2d[idx - Config.RES_LSTM_STEP: idx]
    test_data = test_data2d[idx:idx + day_size * Config.RES_LSTM_TEST_DAYS]
...
def prepare_test_set_last_5days(test_data2d, test_data_normalized2d):
    ...
    init_data_normalize = np.copy(test_data_normalized2d[idx - Config.RES_LSTM_STEP: idx])
