...
from Models.dclstm.dclstm_supervisor import DCLSTMSupervisor


def print_dclstm_att_info(mode, config):
    print('----------------------- INFO -----------------------')
...
def train_dclstm_att(config):
    print('|-- Run model training dclstm.')

    tf_config = tf.ConfigProto()
    tf_config.gpu_options.allow_growth = True

    with tf.Session(config=tf_config) as sess:
        model = DCLSTMSupervisor(is_training=True, **config)
        try:
...
def test_dclstma_att(config):
    print('|-- Run model testing dclstm.')

    tf_config = tf.ConfigProto()
    tf_config.gpu_options.allow_growth = True
    with tf.Session(config=tf_config) as sess:
        model = DCLSTMSupervisor(is_training=False, **config)
        model.load(sess, config['train']['model_filename'])
        model.test(sess)
...
def evaluate_dclstm_att(config):
    print('|-- Run model testing dclstm.')
...
if __name__ == '__main__':
    ...
    print_dclstm_att_info(args.mode, config)
    if args.mode == 'train':
        train_dclstm_att(config)
    elif args.mode == 'evaluate' or args.mode == 'evaluation':
        evaluate_dclstm_att(config)
    else:
        test_dclstma_att(config)
    # get_results(data)
