...
from Models.dcrnn_fwbw_s.dcrnn_supervisor_s import DCRNNSSupervisor


def print_dcrnn_fwbw_s_info(mode, config):
    print('----------------------- INFO -----------------------')
...
def train_dcrnn_fwbw(config):
    ...
    with tf.Session(config=tf_config) as sess:
        model = DCRNNSSupervisor(is_training=True, **config)
        try:
...
def test_dcrnn_fwbw(config):
    ...
    tf_config.gpu_options.allow_growth = True
    with tf.Session(config=tf_config) as sess:
        model = DCRNNSSupervisor(is_training=False, **config)
        model.load(sess, config['train']['model_filename'])
        model.test(sess)
...
if __name__ == '__main__':
    ...
    print_dcrnn_fwbw_s_info(args.mode, config)
    if args.mode == 'train':
